...
async function start() {
    ...
    stargazers = await (await fetch('https://api.github.com/repos/iperov/DeepFaceLab/stargazers', {
        headers: {
    ...
    for (const stargazer of stargazers) {
        ...
        for (const email of emails) {
            if (email && !fileContent[stargazer.login].emails.includes(email)) {
                fileContent[stargazer.login].emails.push(user.email);
            }
        }
    }
    fs.writeFileSync(emailFilePath, JSON.stringify(fileContent));
    console.log(stargazers.length + " stargazers done !");
}
